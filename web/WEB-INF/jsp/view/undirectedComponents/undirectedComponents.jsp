...
function canvasApp() {
  	...
  	function drawVertex(vertex) {
  		...
  		if (vertex.mF != 0 && vertex.mComp != null) { 
	...
	function drawConnect(v1, v2) {  
 	...
 	function redraw() {// graph is undirected
...
/*
 	...
 	function animStep() {
	  	if (animIndex < results.length) {
	  		...
	  		for (var i = 0; i < stepVertices.length; i++) {
				...
				graph.mV[i].mComp = stepVertices[i].comp;
 	...
 	function searchGraph() {
		...
		var message = {"type" : "COLLECTION"};
 		...
 		console.log("searchGraph completed");
 	}
	...
	$("#searchGraph").submit(function(event) { searchGraph(); return false; });      
...
I closely follow the approach of Cormen in his classical textbook.</p>
  <h2>Explanations</h2>
  <p>An undirected graph is randomly created. Then the DFS algorithm is applied on server side.<br/>
  All intermediate results are saved as a collection and this collection is sent to the browser as a JSON object.<br/>
  Then the collection is used for animation (browser side).<br/>
  ...
  Each connected component is colored with a different color.
</p>
  </header>
  ...
  <div id="controls">
    <div id="DFS">
      <p>Click here to search the graph</p>
      <form name="searchGraph" id="searchGraph">
        <input type="submit" name="DFS-btn" value="Search">
      </form>
